...
def extract_colors(image_path):
    """Extracts exactly 5 colors from a given image from left (darkest) to right (lightest)."""
    img = Image.open(image_path).convert("RGB")
    width, height = img.size

    # Define 5 evenly spaced positions to sample colors
    step = width // 5
    sampled_colors = [img.getpixel((step * i + step // 2, height // 2)) for i in range(5)]
...
output_json = "palette.json"  # Output JSON file
