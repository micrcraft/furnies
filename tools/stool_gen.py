...
def get_blockstate(wood):
    return {
        "variants": {
        "facing=north": {
            "model": f"furnies:block/stool/{wood}"
        },
        "facing=east": {
            "model": f"furnies:block/stool/{wood}",
            "y": 90
...
def get_block_model(wood, suffix=""):
    return {
        "parent": f"furnies:block/template/stool{suffix}",
        "textures": {
            "1": f"furnies:block/stool/{wood}"
        }
    }
...
def get_item_model(wood):
    return {
        "parent": "furnies:block/template/stool",
        "textures": {
            "1": f"furnies:block/stool/{wood}"
        }
    }
...
def generate_files():
    ...
    for wood in woods:
        # Blockstate
        blockstate_path = os.path.join(base_dir, f"blockstates/{wood}_stool.json")
        ...
        for suffix in suffixes:
            block_model_path = os.path.join(base_dir, f"models/block/stool/{wood}{suffix}.json")
            save_json_file(block_model_path, get_block_model(wood, suffix))

        # Item model
        item_model_path = os.path.join(base_dir, f"models/item/{wood}_stool.json")
