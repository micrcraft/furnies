...
public class KitchenDrawerBlockEntity extends RandomizableContainerBlockEntity {
    ...
    public KitchenDrawerBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(FBlockEntityTypes.KITCHEN_DRAWER.get(), blockPos, blockState);
        this.items = NonNullList.withSize(27, ItemStack.EMPTY);
        this.openersCounter = new ContainerOpenersCounter() {
            protected void onOpen(Level level, BlockPos pos, BlockState state) {
                KitchenDrawerBlockEntity.this.playSound(SoundEvents.BARREL_OPEN);
                KitchenDrawerBlockEntity.this.updateBlockState(state, true);
            }

            protected void onClose(Level level, BlockPos pos, BlockState state) {
                KitchenDrawerBlockEntity.this.playSound(SoundEvents.BARREL_CLOSE);
                KitchenDrawerBlockEntity.this.updateBlockState(state, false);
            }
            ...
            protected boolean isOwnContainer(Player player) {
                if (player.containerMenu instanceof ChestMenu) {
                    ...
                    return container == KitchenDrawerBlockEntity.this;
                } else {
                    return false;
                }
            }
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.furnies.kitchen_drawer");
    }
    ...
    void updateBlockState(BlockState state, boolean open) {
        this.level.setBlock(this.getBlockPos(), state.setValue(KitchenDrawerBlock.OPEN, open), 3);
    }
