...
public class DrawerBlockEntity extends RandomizableContainerBlockEntity {
    ...
    public DrawerBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(FBlockEntityTypes.DRAWER.get(), blockPos, blockState);
        this.items = NonNullList.withSize(27, ItemStack.EMPTY);
        this.openersCounter = new ContainerOpenersCounter() {
            protected void onOpen(Level level, BlockPos pos, BlockState state) {
                DrawerBlockEntity.this.playSound(SoundEvents.BARREL_OPEN);
                DrawerBlockEntity.this.updateBlockState(state, true);
            }

            protected void onClose(Level level, BlockPos pos, BlockState state) {
                DrawerBlockEntity.this.playSound(SoundEvents.BARREL_CLOSE);
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.furnies.drawer");
    }
