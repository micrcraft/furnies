...
public class FreezerBlockEntity extends RandomizableContainerBlockEntity {
    ...
    public FreezerBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(FBlockEntityTypes.FREEZER.get(), blockPos, blockState);
        this.items = NonNullList.withSize(27, ItemStack.EMPTY);
        this.openersCounter = new ContainerOpenersCounter() {
            protected void onOpen(Level level, BlockPos pos, BlockState state) {
                FreezerBlockEntity.this.playSound(FSoundEvents.FRIDGE_OPEN.get());
                FreezerBlockEntity.this.updateBlockState(state, true);
            }

            protected void onClose(Level level, BlockPos pos, BlockState state) {
                FreezerBlockEntity.this.playSound(FSoundEvents.FRIDGE_CLOSE.get());
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.furnies.freezer");
    }
